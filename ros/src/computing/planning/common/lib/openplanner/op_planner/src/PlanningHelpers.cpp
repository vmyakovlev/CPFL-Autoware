...
bool PlanningHelpers::GetRelativeInfoSlow(const std::vector<WayPoint>& trajectory, const WayPoint& p, RelativeInfo& info, const int& prevIndex )
{
	...
	{
		info.iFront = GetClosestNextPointIndex_obsolete(trajectory, p, prevIndex);
//		WayPoint p2 = p;
...
int PlanningHelpers::GetClosestNextPointIndexFast(const vector<WayPoint>& trajectory, const WayPoint& p,const int& prevIndex )
{
	int size = (int)trajectory.size();
		...
		{
			//std::cout << "Res: " << resolution << ", skip: " << skip << ", i" << i << ", i+skip/2" << i+skip/2 << ", Size: " << size << std::endl;
			if(i+skip/2 < size)
