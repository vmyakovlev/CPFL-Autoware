...
#ifdef HEVSIM
#define ZMP_CLEAR_CNT_DIAG()
#define ZMP_UPDATE_STATE()
#define ZMP_ACCEL_STROKE() GetAccelStroke()
#define ZMP_BRAKE_STROKE() GetBrakeStroke()
#define ZMP_STEERING_TORQUE() GetSteerTorque()
#define ZMP_STEERING_ANGLE() GetSteerAngle()
#define ZMP_VELOCITY() GetVelocity()
#define ZMP_SET_DRV_VELOC(x) SetVelocity((x))
#define ZMP_SET_DRV_STROKE(x) SetAccelStroke((x))
#define ZMP_SET_BRAKE_STROKE(x) SetBrakeStroke((x))
#define ZMP_SET_STR_TORQUE(x) SetSteerTorque((x))
#define ZMP_SET_STR_ANGLE(x) SetSteerAngle((x))
// see AutowareRider
#define SHIFT_POS_B 0
#define SHIFT_POS_D 0x10
#define SHIFT_POS_N 0x20
#define SHIFT_POS_R 0x40
#define ZMP_SET_SHIFT_POS_D() SetDrvShiftMode(SHIFT_POS_D)
#define ZMP_SET_SHIFT_POS_R() SetDrvShiftMode(SHIFT_POS_R)
#define ZMP_SET_SHIFT_POS_B() SetDrvShiftMode(SHIFT_POS_B)
#define ZMP_SET_SHIFT_POS_N() SetDrvShiftMode(SHIFT_POS_N)
#define ZMP_STOP() usleep(2*200000)
#define ZMP_DRV_CONTROLLED() (GetDriveMode() != 0)
#define ZMP_SET_DRV_MANUAL() do { \
    SetDriveMode(0); \
    usleep(2*200000); \
  } while(0)
#define ZMP_SET_DRV_PROGRAM() do { \
    SetDriveMode(0x10); \
    usleep(2*200000); \
  } while(0)
#define ZMP_STR_CONTROLLED() (GetSteerMode() != 0)
#define ZMP_SET_STR_MANUAL() do { \
    SetSteerMode(0); \
    usleep(2*200000); \
  } while(0)
#define ZMP_SET_STR_PROGRAM() do { \
    SetSteerMode(0x10); \
    usleep(2*200000); \
  } while(0)
#else /* HEVSIM */
